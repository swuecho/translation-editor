...
// Header Component
const Header = ({ searchValue, onSearchChange, onSave, onExport, onExportToDirectory, onReset, onConfig }) => {
    return html`
        <div class="header">
            ...
            <div class="controls">
                ...
                <button class="btn save" onClick=${onSave}>Save All</button>
                <button class="btn" onClick=${onExport}>Export JSON</button>
...
const App = () => {
    ...
    const handleExport = useCallback(() => {
        const files = {
            'en-HK-hk.json': JSON.stringify(translations.en, null, 2),
            'hk-HK-hk.json': JSON.stringify(translations.hk, null, 2),
            'zh-HK-hk.json': JSON.stringify(translations.zh, null, 2)
        };
        ...
        showToast('Translation files exported!');
    }, [translations, showToast]);
    ...
    const handleExportToDirectory = useCallback(async () => {
        ...
        if ('showDirectoryPicker' in window) {
            try {
                ...
                const files = [
                    { path: getExportPath(config.paths.en), content: JSON.stringify(translations.en, null, 2) },
                    { path: getExportPath(config.paths.hk), content: JSON.stringify(translations.hk, null, 2) },
                    { path: getExportPath(config.paths.zh), content: JSON.stringify(translations.zh, null, 2) }
                ];
                ...
                showToast('Files exported to selected directory!');
            ...
            showToast('Directory selection not supported in this browser. Using regular download.', 'error');
            handleExport();
        }
    }, [translations, config, showToast, handleExport, setOriginalTranslations, setModifiedKeys]);
